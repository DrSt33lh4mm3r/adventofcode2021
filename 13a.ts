...
export class Puzzle_13a extends Puzzle {
    paper: string[] = [];
    ...
    processLines(lines: string[]) {
        ...
        for (let i = 0; i < 1/*foldingInstructions.length*/; i++) {
    ...
    countOnes(one: string = '#') {
        const joined = this.paper.join('');
        const ones = joined.split('').filter(c => c === one);
