...
export class Puzzle_17a extends Puzzle {
    ...
    processLines(lines: string[]) {
        ...
        // this.x = {min: 20, max: 30};
        // this.y = {min: -10, max: -5};

        let possibleXValues: number[] = [0];
        ...
        for (let i = 1; i <= this.x.max + 1; i++) {
            let x = i;

            let currX = 0;
            let hitTarget = false;

            while (currX <= this.x.max && x > 0) {
                currX = currX + x;
        ...
        // console.log(hits.map(h => '' + h.x + ';' + h.y).join('#'));
    ...
    simulateThrow(x: number, y: number) {
        ...
        while (currX <= this.x.max && currY >= this.y.min) {
            currX = currX + x;
            currY = currY + y;
